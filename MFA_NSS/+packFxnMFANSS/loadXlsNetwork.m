...
function model = loadXlsNetwork(model,optionsMFA)

%% Load reaction information
[~,~,rawXlsData] = xlsread(optionsMFA.xlsModelFileName,'Reactions');
...
for i = 1 : size(rawXlsData,2)
    switch colNames{i}
        ...
        case {'Lower bound of forward/backward fluxes'}
            tmpRxnInfo.lbFluxes = cell2mat(rawXlsData(:,i));            
        case {'Upper bound of forward/backward fluxes'}
            tmpRxnInfo.ubFluxes = cell2mat(rawXlsData(:,i));            
        case {'Note'}
...
rxnInfo.lbFluxes = zeros(nRxns,1);
rxnInfo.ubFluxes = zeros(nRxns,1);
...
for i = 1 : nTmpRxns
    if tmpRxnInfo.isRev(i)
        ...
        rxnInfo.lbFluxes(ii) = tmpRxnInfo.lbFluxes(i);
        rxnInfo.ubFluxes(ii) = tmpRxnInfo.ubFluxes(i);
...
[~,~,xlsMetData] = xlsread(optionsMFA.xlsModelFileName,'Metabolites');
...
for i = 1 : size(xlsMetData,2)
    switch colNames{i}
        ...
        case {'Compartment IDs'}
            model.metInfo.compt = cell2mat(xlsMetData(2:end, i));
        ...
        case {'Lower bound of metabolite concentrations'}
            model.metInfo.lbConcs = cell2mat(xlsMetData(2:end, i));
        case {'Upper bound of metabolite concentrations'}
            model.metInfo.ubConcs = cell2mat(xlsMetData(2:end, i));
        case {'Lower bound of concentration changes over time'}
            model.metInfo.lbConcRates = cell2mat(xlsMetData(2:end, i));
        case {'Upper bound of concentration changes over time'}
            model.metInfo.ubConcRates = cell2mat(xlsMetData(2:end, i));
        case {'Note'}
...
% NEED to be removed
tmpMet = 'Glc_ex';
idMet = find(strncmp({tmpMet}, model.metInfo.mets, length(tmpMet)));
model.metInfo.massType(idMet) = 1;
tmpMet = 'Pyr';
idMet = find(strncmp({tmpMet}, model.metInfo.mets, length(tmpMet)) ...
    & cellfun(@isempty, strfind(model.metInfo.mets, '_ex')));
model.metInfo.massType(idMet) = -1;
...
for i = 1 : nMets
    ...
    useMassMat(i,1:model.metInfo.nCarbonMets(i)+1) = true;
end
